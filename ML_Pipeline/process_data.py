...
def merge_data():
    ...
    # Select data for India from the world vaccination dataset
    df_world = df_world[df_world.location == 'India']
    
    # Change dtype of date for efficient merging
    df_world.date = pd.to_datetime(df_world.date)
    df_india.date = pd.to_datetime(df_india.date)

    # Merging datasets
    df = df_world.merge(df_india[['date', 'vaccine', 'source_url']], left_on='date', right_on='date', how='outer')

    # Final columns list - Rearranging columns 
    cols = ['location',
     'date',
     'vaccine',
     'source_url',
